...
CHANNEL_ID = "193"
KEYWORD = "信息披露文件"
OUTPUT_BASE_DIR = "./province_bonds_disclosure"
...
def download_province(province, ad_code, max_pages=20):
    print(f"\n{'=' * 50}", flush=True)
    print(f"开始抓取【{province}】{KEYWORD}...", flush=True)
...
def main():
    print("=" * 50, flush=True)
    print(f"只下载河北、河南含'{KEYWORD}'的PDF", flush=True)
    print("=" * 50, flush=True)

    total = 0
    for province, ad_code in PROVINCE_CODES.items():
        total += download_province(province, ad_code, max_pages=20)
        time.sleep(1)
